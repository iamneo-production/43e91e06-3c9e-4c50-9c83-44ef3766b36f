...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.authorizeRequests((request)->request.antMatchers("h2-console/**","/api/v1/auth/login","api/v1/auth/dashbord","/auth/userinfo","/users/addUser","/users/addAuthority","/users/addAuth"
				...
				"/course/institute/{instituteid}","/course/institutes/{instituteid}","/users/{username}","/student/viewStudents","/student/addStudent","/student/{studentid}","/student/updateStudent/{studentid}","/student/particular/{id}").permitAll().antMatchers(HttpMethod.OPTIONS,"/**").permitAll().anyRequest().authenticated())
